...
if (isset($_POST['btnguardar'])) {
    ...
    if (!empty($id) && !empty($nombre) && !empty($apellido) && !empty($fecha)) {
        ...
        if ($stmt = $conexion->prepare("UPDATE empleado SET nombre = ?, apellido = ?, fechadenac = ? WHERE idempleado = ?")) {
            ...
            if ($stmt->affected_rows > 0) {
                ...
                header("Location: ../vista/index.php"); // Ajusta la ruta si es necesario
                exit();
            } else {
